 ...
 * 
 *  ConfigUser.java is free software; you can redistribute it and/or
...
*/
	...
	//{
	  if(value instanceof String) settings.put(key,(String) value);
	  else if(value instanceof Integer) settings.putInt(key,((Integer) value).intValue());
...
*/
	...
	if(configHandler == null) { configHandler = new ConfigHandler(args); setConfHandler(configHandler);}
        return(args);
    }//public void init
    /**
...
*/
  protected Preferences parseArgs(String argv[])
  {
	...
	if((i+1) < argv.length) next = argv[i+1].trim();
	if( tmp.startsWith("-")) 
	{
	  //process switch 
